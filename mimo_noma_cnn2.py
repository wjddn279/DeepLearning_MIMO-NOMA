...
def reciever(input1, num):
    with tf.variable_scope("rx" + str(num)):
        ...
        input_flat = tf.reshape(pool2, [-1, 8 * 8 * 8])

        input1 = tf.contrib.layers.batch_norm(input_flat, center=True, scale=True)
        dense1 = tf.layers.dense(input1, 1000 * modulation_order, activation=tf.nn.relu,
        ...
        dense2 = tf.layers.dense(dense1, 1000 * modulation_order, activation=tf.nn.relu,
                                 kernel_initializer=tf.initializers.truncated_normal(stddev=0.01))
        dense2 = tf.contrib.layers.batch_norm(dense2, center=True, scale=True)
        dense3 = tf.layers.dense(dense2, 1000 * modulation_order, activation=tf.nn.relu,
                                 kernel_initializer=tf.initializers.truncated_normal(stddev=0.01))
        dense3 = tf.contrib.layers.batch_norm(dense3, center=True, scale=True)

        dense4 = tf.layers.dense(dense3, 100 * modulation_order, activation=tf.nn.relu,
                                 kernel_initializer=tf.initializers.truncated_normal(stddev=0.01))
...
square_input1 = tf.reshape(in1, [-1, 8, 8, 1])
...
square_input2 = tf.reshape(in2, [-1, 8, 8, 1])
